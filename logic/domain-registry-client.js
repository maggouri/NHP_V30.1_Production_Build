...
/**
 * EP-302C — Domain registry UI helpers (CJS test mirror of domain-registry-helpers.js).
 */
/** Creaty :3020 paths — legacy mailbox-lifecycle mount (not used by Quick Access after INT-002). */
const DOMAIN_REGISTRY_API = Object.freeze({
...
const EMAILCORE_DOMAIN_REGISTRY_API = Object.freeze({
...
const DOMAIN_REGISTRY_ACTIONS = Object.freeze({
  VERIFY: 'verify',
  UNVERIFY: 'unverify',
  ENABLE: 'enable',
  DISABLE: 'disable',
...
function canManageDomainRegistry(role) {
  return String(role || '').trim() === 'Admin';
}

function resolveDomainStatusLabelAr(status) {
  return STATUS_LABELS_AR[String(status || '').trim()] || '—';
}

function resolveDomainVerifiedLabelAr(isVerified) {
  return isVerified ? 'مُتحقق' : 'غير مُتحقق';
}

function validateDomainNameInput(raw) {
...
function mapDomainRegistryError(envelope = {}, fallback = '') {
...
function sortDomainsForDisplay(domains = []) {
