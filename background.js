...
function decodeBase64ToUint8Array(base64OrDataUrl, fallbackMime = 'image/png') {
    ...
    if (raw.startsWith('data:')) {
        ...
        mimeType = (metadata.split(';')[0] || mimeType).trim() || 'image/png';
...
function readImageDimensionsFromBytes(bytes, mimeType = 'image/png') {
    if (!(bytes instanceof Uint8Array) || bytes.length < 24) return null;
    const mime = String(mimeType || '').toLowerCase();

    // PNG: IHDR width/height at bytes 16..23
    if (
        (mime.includes('png') || (bytes[0] === 0x89 && bytes[1] === 0x50)) &&
    ...
    if (mime.includes('jpeg') || mime.includes('jpg') || (bytes[0] === 0xff && bytes[1] === 0xd8)) {
        ...
        while (offset + 9 < bytes.length) {
            ...
            const segmentLength = ((bytes[offset + 2] << 8) | bytes[offset + 3]) >>> 0;
            if (segmentLength < 2 || offset + 2 + segmentLength > bytes.length) break;
            // SOF0..SOF3 / SOF5..SOF7 / SOF9..SOF11 / SOF13..SOF15 (skip DHT/DAC)
...
async function getImageDimensionsFromBase64(base64Data, mimeType = 'image/png') {
    // MV3 connect-src blocks fetch('data:...') — decode locally (no network).
    const decoded = decodeBase64ToUint8Array(base64Data, mimeType || 'image/png');
    if (!decoded?.bytes?.length) return null;

    try {
        const headerDims = readImageDimensionsFromBytes(decoded.bytes, decoded.mimeType);
        if (headerDims) {
            return { width: headerDims.width, height: headerDims.height, bytes: decoded.bytes.length };
        }

        const blob = new Blob([decoded.bytes], { type: decoded.mimeType || 'image/png' });
        const bitmap = await createImageBitmap(blob);
        const dimensions = { width: bitmap.width, height: bitmap.height, bytes: decoded.bytes.length };
